...
def content_diff(ladderon0, content): # compare the content difference between two strings
    content_diff0 = {}
    c0 = dict(Counter(ladderon0))
    c1 = dict(Counter(content))
    for letter, n in c0.items():
        if letter in c1:
            left = n - c1[letter]
            if left > 0:
                content_diff0[letter] = left
        else:
            content_diff0[letter] = n
    return content_diff0   # {'E': 1, 'D': 1, ...} ~ {Ladderon: #Left, ...}
...
class STRMAT(object):
    ...
    def laddergraph(self, filename = "G", show_longer_than = 0, style = "ellipse", color = "grey", rankdir = "BT"):
        ...
        ladderonDF = {}
        for i, seq in enumerate(self.strs):
            ladderonDF[i] = [seq, '']
        for key, value in binfo.items():
            if key in self.strs:
                ladderonDF[value[0]] = [key, self.strs.index(key)]
            else:
                ladderonDF[value[0]] = [key, '']
        ladderonListABOVE = [] # all ladderons except basic building blocks
        for key, value in ladderonDF.items():
            ladderonListABOVE.append([key, value[0], value[1]])
        templevel0 = [] # IDs of the basic building blocks
        i = 0
        for key in self.ladderonBookLevel0.keys():
            i -= 1
            templevel0 = [[i, key, '']] + templevel0
        ladderonList = templevel0 + ladderonListABOVE # add basic building blocks
        ladderonDF = pd.DataFrame(ladderonList, columns = ['ID', 'Ladderon', 'ID-also'])
        ...
        if style == "box": # a detailed version, showing ladderons length
            ...
            for i, seq in enumerate(self.strs): # draw all target sequences
                if seq in binfo:
                    g.node(str(i), label = seq + ' (&)')
                else:
                    g.node(str(i), label = seq)
            if show_longer_than == 0: # display basic building blocks or not
                for ID, basicLadderon, _ in templevel0:
                    g.node(str(ID), label = basicLadderon, shape = 'hexagon', style = 'filled', color=color)
            ...
            for i, seq in enumerate(self.strs):
                ...
                if seq in binfo:
                    IDanother = binfo[seq][0]
                    templabel = str(i) + '(' + str(IDanother) + ')'
                    g.node(str(i), label = templabel, **{'width':str(length),'height':str(length/2)})
                else:
                    g.node(str(i), label = str(i), **{'width':str(length),'height':str(length/2)})
            if show_longer_than == 0: # display basic building blocks or not
                length = ellipse_len(templevel0[0][1])
                for ID, basicLadderon, _ in templevel0:
        ...
        for key, value in binfo.items():
            ...
            for splice in value[1:]:
                if len(key) > show_longer_than:
                    if style == "box":
                        g.node(str(value[0]), label = str(key), style = 'filled', color=color)
                    else: # style == "ellipse":
                        lenk = ellipse_len(key)
                        g.node(str(value[0]), label = str(value[0]), **{'width':str(lenk),'height':str(lenk/2)}, \
            ...
            if style == "box":
                ...
                for vp, times in dict_temp.items():
                    for j in range(times):
                        ...
                        g.edge(vp[0], vp[1], color = color)

        if show_longer_than == 0: # display the links from basic building blocks
            for ID0, ladderon0, _ in ladderonListABOVE: # check every ladderon
                if (ID0 < len(self.strs)) and len(ladderonDF[ladderonDF['Ladderon'] == ladderon0]) > 1: # exclude targets those have 2 IDs
                    pass
                else:
                    content = ''
                    for thisLadderon, val in self.ladderonBook.items():
                        if thisLadderon != ladderon0: # span all ladderons except itself
                            for i, _, j, _ in val[1:]:
                                if i == ID0:
                                    content += thisLadderon
                                if j == ID0:
                                    content += thisLadderon
                    Content_Diff = content_diff(ladderon0, content) # {'E': 1, 'D': 1, ...} ~ {Ladderon: #Left, ...}
                    if len(Content_Diff) > 0:
                        for letter, left in Content_Diff.items():
                            ID = int( ladderonDF[ladderonDF['Ladderon'] == letter]['ID'] )
                            for j in range(left):
                                if style == "box":
                                    g.edge(str(ID), str(ID0))
                                else: # style == "ellipse":
                                    g.edge(str(ID), str(ID0), color = color)
        g.view()
        return ladderonDF
...
# strsInput = ['ABCAB', 'BACAX', 'BACAX'] or strsInput = {'ABCAB': 2, 'BACAX': 1, 'BACAX': 1}
    ...
    if hasDup: 
        print('------ Warning: There are duplications in the targets!!! -----------------------------------------')
        print('------          Although we can calculate the indices, the laddergraph is a bit ambiguous.  ------')
        print('------          To avoid confusion, remove the duplications in the targets. ----------------------')
        print('')
