...
def change_word(a,eta_random_time=10):
    total_list = []
    word_index = [1,3,5,7,9]
    for i in range(len(a)-1):
        qwe = np.array([1,3,5,7,9])+(i+1)*10+i+1
        word_index = word_index+qwe.tolist() ## Get the index of the Chinese character sequence
    for i in range(len(a)):
        for j in range(len(a[i])):
            total_list.append(a[i][j]) # Get a list of all characters
    total_array = np.array(total_list) 
    word_array = total_array[word_index]
   # random.seed(seed)
    list_change_total = []
    order_total = []
    for i in range(eta_random_time):#Number of times to randomly pick the minimum value
        
        random.shuffle(word_array)#Randomly arrange Chinese characters
        total_array[word_index] = word_array #  Replace the arranged Chinese characters

        list_change = []
        for i in range(len(a)):
            str_name = total_array[11*i]+total_array[11*i+1]+total_array[11*i+2]+total_array[11*i+3]+total_array[11*i+4]+total_array[11*i+5]\
            +total_array[11*i+6]+total_array[11*i+7]+total_array[11*i+8]+total_array[11*i+9]+total_array[11*i+10]
...
def max_order(a):
   ...
   # random.seed(1)
    #random.shuffle(word_array)# Randomly arrange Chinese characters
    total_array[word_index] = word_array[0] # Replace the arranged Chinese characters
...
def create_model(x,y,period,epochs,min_maxval=[-1,1],lr=0.25,layer123=[8,15,2,6,2],seed=1,seed_1=7,seed_2=10,seed_3=12,seed_4=1,seed_5=1,input_dim=3):# period represents how often to save, epochs is the number of training rounds
    import random
    #random.seed(seed)
   # np.random.seed(seed)
    tf.random.set_seed(seed)
    model = Sequential()
    model.add(tensorflow.keras.layers.Dense(layer123[0], input_dim=input_dim,kernel_initializer=keras.initializers.RandomUniform(minval=min_maxval[0], maxval=min_maxval[1], seed=seed_1)))## Specify the uniform distribution initialization of the lower and upper bounds
...
#,kernel_initializer='ones',bias_initializer='zeros'))
    ...
    model.add(tensorflow.keras.layers.Activation('relu'))
    model.add(tensorflow.keras.layers.Dense(layer123[4],kernel_initializer=keras.initializers.RandomUniform(minval=min_maxval[0], maxval=min_maxval[1], seed=seed_5)))
...
#    metrics=["accuracy"],)
    ...
    #  Save weights in the `checkpoint_path` format
    ...
    layer_name = ['layer_with_weights-0/kernel/.ATTRIBUTES/VARIABLE_VALUE',
                 ...
                 'layer_with_weights-3/kernel/.ATTRIBUTES/VARIABLE_VALUE',
                  ...
                  'layer_with_weights-4/bias/.ATTRIBUTES/VARIABLE_VALUE'
                 ]
...
def grouping_weight(data,bins=0.1): # Pass in the w list that needs to be coarse-grained, such as 10 groups of parameters, and coarse-grain the 0th, 2nd, 4th layer of each group of parameters
...
def result_acc(predict_array,true_label):  #predict_array is the output of the model after giving the dataset, such as model(X_test)
    predict_label = []
...
# The following function calls grouping_weight to calculate the accuracy of the coarse-grained network after passing the test set through grouping.
# The first loop is the loop of the coarse-grained interval, starting from interval 0.05 (for this problem, our bins are fixed at 0.1).
# The second loop is the loop of different networks under each coarse-grained interval, with a total of 100 networks, each calculating an accuracy.


def test_bin(weight,acc_X,acc_y,bin_list=[0.1],layer123=[8,15,2,2,2],input_dim=3):
    acc_list = []
    ...
    for bins in  bin_list:
        ...
        model_int.add(tensorflow.keras.layers.Activation('relu'))
        model_int.add(tensorflow.keras.layers.Dense(layer123[4]))
        ...
        for j in range(len(weight_copy_grouping)):
            ...
            #model_int.set_weights((np.array(weight_copy_grouping[j])).tolist())
            model_int.set_weights((np.array(weight_copy[j])).tolist())
...
# #  Define the function to label the weights

def print_list_letter_path(weight,ming_unique,bins=0.4):  #  Here the input is the network before coarse-graining, and bins are used for labeling
    list_letter = []   # Label the weights after coarse-graining
    for i in range(5):
        list_w = []
        w = weight[i*2].flatten()
        for j in range(len(w)):
            num = int(np.round((w[j]-0)/bins))# Give the coarse-grained weights an index: such as 0.15, 0.15/0.1=1.5 rounded to 2, then the index of 0.15 is 2, similar indices for 0.17, 0.18 are also 2
            #list_w.append('A_'+str(num))
            index = 500+num #After obtaining the index, each index corresponds to a Chinese character, the mapping for weights of 0 is the 500th character, the first weight larger than 0 is mapped to the 501st character, and so on...
...
#ming_unique
    ...
    path_list = [] #  Traverse to get all paths after labeling
                        ...
                        for q in range(len(list_letter[2][k])):
                            if list_letter[2][k][q] != ming_unique[500]:
                                for p in range(len(list_letter[3][q])):
                                    if  list_letter[3][q][p] != ming_unique[500]: # Up to this point, the above loop code is to remove the weights of 0 in each layer
                                        for z in range(len(list_letter[4][p])):
                                            if list_letter[4][p][z] != ming_unique[500]:   
                                                path_list.append([list_letter[0][i][j],list_letter[1][j][k],list_letter[2][k][q],list_letter[3][q][p],list_letter[4][p][z]])
