...
def change_word(a,eta_random_time=10):
    ...
    for i in range(len(a)-1):
        qwe = np.array([1,3,5,7])+(i+1)*8+i+1
        word_index = word_index+qwe.tolist() # Get the index sequence of Chinese characters
    for i in range(len(a)):
        for j in range(len(a[i])):
            total_list.append(a[i][j]) # Get the list of all characters separated
    total_array = np.array(total_list) 
    word_array = total_array[word_index]
   # random.seed(seed)
    list_change_total = []
    order_total = []
    for i in range(eta_random_time):# Indicates the number of times to randomly take the minimum value
        
        random.shuffle(word_array)#  Shuffle the Chinese characters randomly
        total_array[word_index] = word_array # Replace the shuffled Chinese characters

        list_change = []
        for i in range(len(a)):
            str_name = total_array[9*i]+total_array[9*i+1]+total_array[9*i+2]+total_array[9*i+3]+total_array[9*i+4]+total_array[9*i+5]+total_array[9*i+6]+total_array[9*i+7]+total_array[9*i+8]
...
def max_order(a):
    ...
    random.seed(1)
   # random.shuffle(word_array)#Shuffle the Chinese characters randomly
    total_array[word_index] = word_array[0] #Replace with the first Chinese character after shuffling
...
def create_model(x,y,period,epochs,min_maxval=[-1,1],lr=0.25,layer123=[8,15,4,2],seed=1,seed_1=7,seed_2=10,seed_3=12,seed_4=1,input_dim=3):#period represents how often to save, epochs is the number of training rounds
    import random
    #random.seed(seed)
   # np.random.seed(seed)
    tf.random.set_seed(seed)
    model = Sequential()
    model.add(tensorflow.keras.layers.Dense(layer123[0], input_dim=input_dim,kernel_initializer=keras.initializers.RandomUniform(minval=min_maxval[0], maxval=min_maxval[1], seed=seed_1)))## Specify a uniform distribution initialization with lower and upper bounds
...
#,kernel_initializer='ones',bias_initializer='zeros'))
    ...
    model.add(tensorflow.keras.layers.Activation('relu'))
    model.add(tensorflow.keras.layers.Dense(layer123[2],kernel_initializer=keras.initializers.RandomUniform(minval=min_maxval[0], maxval=min_maxval[1], seed=seed_3)))
    model.add(tensorflow.keras.layers.Activation('relu'))
    model.add(tensorflow.keras.layers.Dense(layer123[3],kernel_initializer=keras.initializers.RandomUniform(minval=min_maxval[0], maxval=min_maxval[1], seed=seed_4)))
...
#    metrics=["accuracy"],)
    ...
    # Create a callback to save the model's weights every 'period' epochs
    cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path,verbose=0,save_weights_only=True,period=period)
    ...
    layer_name = ['layer_with_weights-0/kernel/.ATTRIBUTES/VARIABLE_VALUE',
                  ...
                  'layer_with_weights-2/bias/.ATTRIBUTES/VARIABLE_VALUE',
                'layer_with_weights-3/kernel/.ATTRIBUTES/VARIABLE_VALUE',
                  'layer_with_weights-3/bias/.ATTRIBUTES/VARIABLE_VALUE']
...
def grouping_weight(data,bins=0.1): # Pass in the w list that needs to be coarse-grained, such as grouping the 0th, 2nd, 4th layers of each group of parameters with a total of 10 groups
...
def result_acc(predict_array,true_label):  # predict_array is the model output after providing the dataset, e.g., model(X_test)
    predict_label = []
    ...
    # print('Data total:', total_num)
    # print('Predicted correctly:', predict_right_num)
    # print('Accuracy:', acc)
    return acc


# The following function calls grouping_weight to calculate the accuracy of the coarse-grained network
# Pass the test set into the grouped network. The first loop is the loop of the coarse-grained interval, starting from 0.05 (we fixed the bins to 0.1 for this problem)
# The second loop is the loop of different networks under each coarse-grained interval. There are a total of 100 networks, and each network calculates an accuracy.




def test_bin(weight,acc_X,acc_y,bin_list=[0.1],layer123=[8,15,2,2],input_dim=3):
    acc_list = []
    ...
    for bins in  bin_list:
        ...
        model_int.add(tensorflow.keras.layers.Activation('relu'))
        model_int.add(tensorflow.keras.layers.Dense(layer123[2]))
        model_int.add(tensorflow.keras.layers.Activation('relu'))
        model_int.add(tensorflow.keras.layers.Dense(layer123[3]))
        model_int.add(tensorflow.keras.layers.Activation('softmax'))
        
        weight_copy_grouping = grouping_weight(data=weight_copy,bins=bins) # Call to coarse-grain the w list, such as grouping the 0th, 2nd, 4th layers of each group of parameters
        acc_mean_array = np.zeros([len(weight_copy_grouping),1])
        for j in range(len(weight_copy_grouping)):
            
            #model_int.set_weights((np.array(weight_copy_grouping[j])).tolist())# Pass in the coarse-grained network to calculate acc
            model_int.set_weights((np.array(weight_copy[j])).tolist()) # Pass in the original network to calculate acc
            acc = result_acc(predict_array=model_int.predict(acc_X),true_label=acc_y)
           # acc_mean_array[j] = acc
            
            acc_list.append(acc)
       # acc_mean = np.mean(acc_mean_array)
       # print('Interval is:', bins, 'Average acc is:', acc_mean)
       # acc_bin_mean_list.append(acc_mean)
        grouping_weight_list.append(weight_copy_grouping)
        return acc_list,grouping_weight_list

# # Define weight labeling

def print_list_letter_path(weight,ming_unique,bins=0.4):  # Here, pass in the network before coarsening, and use bins for labeling
    list_letter = []  # Label the weights after coarsening the network
    for i in range(4):
        list_w = []
        w = weight[i*2].flatten()
        for j in range(len(w)):
            num = int(np.round((w[j]-0)/bins))# # Give an index to the coarsened weight value: such as 0.15, 0.15/0.1=1.5 rounded to 2, then the index of 0.15 is 2, similar to the index of 0.17, 0.18 is 2
            #list_w.append('A_'+str(num))
            index = 500+num ## After obtaining the index, each index corresponds to a Chinese character, the mapping of the weight of 0 to the 500th character, and the first weight larger than 0 to the 501st character...
            list_w.append(ming_unique[index])

        list_w_reshape = np.array(list_w).reshape(weight[i*2].shape[0],weight[i*2].shape[1])# Change it back to the original shape
        list_letter.append(list_w_reshape)
#ming_unique
        
    path_list = [] #After labeling, traverse the network to get all paths
                ...
                for k in range(len(list_letter[1][j])):
                    if list_letter[1][j][k] != ming_unique[500]:
                        ...
                        for q in range(len(list_letter[2][k])):
                            if list_letter[2][k][q] != ming_unique[500]:## Up to this point, the above loop code is to remove the weights of 0 in each layer
                                for p in range(len(list_letter[3][q])):
                                    if  list_letter[3][q][p] != ming_unique[500]:
                                        path_list.append([list_letter[0][i][j],list_letter[1][j][k],list_letter[2][k][q],list_letter[3][q][p]])
    return list_letter,path_list

#Define Chinese character dictionary. Similarly, and return the code
def load_ming_dic():
