...
def change_word(a,eta_random_time=10):
   ...
   # random.seed(seed)
    ...
    for i in range(eta_random_time):# Indicate the number of times to randomly select the minimum value
        ...
        list_change = []
        for i in range(len(a)):
            str_name = total_array[7*i]+total_array[7*i+1]+total_array[7*i+2]+total_array[7*i+3]+total_array[7*i+4]+total_array[7*i+5]+total_array[7*i+6]
...
def create_model(x,y,period,epochs,min_maxval=[-1,1],lr=0.25,layer123=[8,15,2],seed=1,seed_1=7,seed_2=10,seed_3=12,input_dim=3):
    import random
    #random.seed(seed)
   # np.random.seed(seed)
    tf.random.set_seed(seed)
    model = Sequential()
    model.add(tensorflow.keras.layers.Dense(layer123[0], input_dim=input_dim,kernel_initializer=keras.initializers.RandomUniform(minval=min_maxval[0], maxval=min_maxval[1], seed=seed_1)))
...
#    metrics=["accuracy"],)
    ...
    layer_name = ['layer_with_weights-0/kernel/.ATTRIBUTES/VARIABLE_VALUE','layer_with_weights-0/bias/.ATTRIBUTES/VARIABLE_VALUE','layer_with_weights-1/kernel/.ATTRIBUTES/VARIABLE_VALUE','layer_with_weights-1/bias/.ATTRIBUTES/VARIABLE_VALUE','layer_with_weights-2/kernel/.ATTRIBUTES/VARIABLE_VALUE','layer_with_weights-2/bias/.ATTRIBUTES/VARIABLE_VALUE']
    ...
    for i in range(int(model_number)):
        ...
        checkpoint_path = r'./weight_1/cp-'+str(period*i+period).zfill(4)+'.ckpt' 
...
def test_bin(weight,acc_X,acc_y,bin_list=[0.1],layer123=[8,15,2],input_dim=3):
    acc_list = []
...
def print_list_letter_path(weight,ming_unique,bins=0.4):  
    ...
    for i in range(3):
        ...
        list_w_reshape = np.array(list_w).reshape(weight[i*2].shape[0],weight[i*2].shape[1])
                ...
                for k in range(len(list_letter[1][j])):
                    if list_letter[1][j][k] != ming_unique[500]:
                        ...
                        for q in range(len(list_letter[2][k])):
                            if list_letter[2][k][q] != ming_unique[500]:#Up to this point, the above loop code is for removing weights equal to 0 in each layer
                                path_list.append([list_letter[0][i][j],list_letter[1][j][k],list_letter[2][k][q]])
